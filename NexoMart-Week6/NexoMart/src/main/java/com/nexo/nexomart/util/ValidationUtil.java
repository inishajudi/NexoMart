...
public final class ValidationUtil {
    ...
    public static void requireRange(Integer value, int min, int max, String field) throws ValidationException {
        if (value == null || value < min || value > max) {
            throw new ValidationException(field, field + " must be between " + min + " and " + max);
        }
    }

    public static void requireMaxLength(String value, int maxLength, String field) throws ValidationException {
        if (value != null && value.length() > maxLength) {
            throw new ValidationException(field, field + " must not exceed " + maxLength + " characters");
        }
    }
}
