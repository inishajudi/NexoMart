...
public class ProductService {
    ...
    /** Kept for existing callers (Week 2). Prefer {@link #browse(ProductSearchCriteria)}. */
    public List<ProductDTO> browse(String keyword, String category) throws ValidationException {
        return browse(ProductSearchCriteria.empty().keyword(keyword).category(category));
    }

    /** Week 5: adds price range + sort on top of Week 2's keyword/category search. */
    public List<ProductDTO> browse(ProductSearchCriteria criteria) throws ValidationException {
        if (criteria.getMinPrice() != null && criteria.getMaxPrice() != null
                && criteria.getMinPrice().compareTo(criteria.getMaxPrice()) > 0) {
            throw new ValidationException("minPrice", "minPrice must not be greater than maxPrice");
        }
        return productDAO.search(criteria).stream()
                .map(this::toDTO)
    ...
    public ProductDTO update(long sellerId, long productId, ProductDTO dto)
        ...
        ValidationUtil.requireMaxLength(dto.getName(), 150, "name");
        ValidationUtil.requireMaxLength(dto.getDescription(), 2000, "description");
        ValidationUtil.requireMaxLength(dto.getCategory(), 100, "category");
    ...
    /** Admin moderation (F7): remove any listing regardless of owner. Callers MUST verify
     *  the requesting session has role ADMIN before invoking this — this method itself
     *  performs no authorization check. */
    public void adminDelete(long productId) throws NotFoundException {
