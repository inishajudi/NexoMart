...
const NexoMart = (() => {
        ...
        try {
            ...
            grid.innerHTML = products.map(p => `
                <div class="product-card">
                    <h3><a href="/jsp/product-detail.jsp?id=${p.id}">${escapeHtml(p.name)}</a></h3>
    ...
    // ---------- Product detail + reviews (Week 6: F8) ----------
    ...
    async function loadProductDetail(productId) {
        ...
        try {
            const { product, rating } = await apiFetch(`/api/v1/products/${productId}`);
            detailEl.innerHTML = `
                <h2>${escapeHtml(product.name)}</h2>
                <p>${escapeHtml(product.category || '')}</p>
                <p class="price">₹${Number(product.price).toFixed(2)}</p>
                <p>${escapeHtml(product.description || '')}</p>
                <p>${product.stockQty} in stock</p>
                <button id="addToCartDetailBtn" data-product-id="${product.id}">Add to cart</button>
            `;
            document.getElementById('addToCartDetailBtn').addEventListener('click', () => addToCart(productId));

            summaryEl.innerHTML = rating.reviewCount > 0
                ? `<p>${starString(rating.averageRating)} ${rating.averageRating.toFixed(1)} out of 5 (${rating.reviewCount} review${rating.reviewCount === 1 ? '' : 's'})</p>`
                : '<p>No reviews yet.</p>';
        } catch (err) {
            detailEl.innerHTML = `<p>Could not load product: ${escapeHtml(err.message)}</p>`;
        }
    }
    ...
    async function loadReviews(productId) {
        ...
        try {
            const { reviews } = await apiFetch(`/api/v1/reviews?productId=${productId}`);
            if (reviews.length === 0) {
                listEl.innerHTML = '';
                return;
            }
            listEl.innerHTML = reviews.map(r => `
                <div class="review">
                    <strong>${starString(r.rating)}</strong>
                    <p>${escapeHtml(r.comment || '')}</p>
                    <small>${r.createdAt ? new Date(r.createdAt).toLocaleDateString() : ''}</small>
                </div>
            `).join('');
        } catch (err) {
            listEl.innerHTML = `<p>Could not load reviews: ${escapeHtml(err.message)}</p>`;
        }
    }

    async function submitReview(productId, event) {
        event.preventDefault();
    ...
    function initProductDetailPage(productId) {
        loadProductDetail(productId);
        loadReviews(productId);
        document.getElementById('reviewForm').addEventListener('submit', e => submitReview(productId, e));
    }
    ...
    return { initBrowsePage, initCartPage, initSellerDashboard, initAdminDashboard, initMyOrdersPage, initProductDetailPage };
})();
