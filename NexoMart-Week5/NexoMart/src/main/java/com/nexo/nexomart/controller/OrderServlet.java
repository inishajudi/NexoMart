 ...
 * PUT  /api/v1/orders/{id}/status     -> O2 status workflow, body {status: "CONFIRMED"}
 *                                          seller (owns an item in the order) or admin
 *                                          advances; buyer (owns the order) may cancel
...
public class OrderServlet extends HttpServlet {
    ...
    protected void doPut(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        if (pathInfo == null || !pathInfo.endsWith("/status")) {
            JsonUtil.writeJson(resp, HttpServletResponse.SC_NOT_FOUND,
                    ApiResponse.fail("NOT_FOUND", "Unknown order route"));
            return;
        }
        ...
        try {
            ...
            StatusRequest body = JsonUtil.fromRequestBody(req, StatusRequest.class);
            OrderDTO updated = orderService.advanceStatus(orderId, body != null ? body.status : null, userId, isAdmin);
            ...
            JsonUtil.writeJson(resp, HttpServletResponse.SC_FORBIDDEN,
                    ApiResponse.fail("FORBIDDEN", e.getMessage()));
        }
    }
    ...
    /** Body shape for PUT /api/v1/orders/{id}/status. */
